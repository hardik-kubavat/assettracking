...
from models import LocaterModel,ProductTypeModel,CategoryModel,ProductModel

rv = Blueprint('report','report',url_prefix="/report")

@rv.route('/locaterwise',methods=['GET'])
def generate_locater_wise_report():
    html = render_template("locaterreport.html",locaters = LocaterModel.getAll())
...
def generate_producttype_wise_report():
    html = render_template("producttypereport.html",producttypes = ProductTypeModel.getAll())
...
def generate_category_wise_report():
    html = render_template("categoryreport.html",categories = CategoryModel.getAll())
...
def generate_notworking_asset_report():
    html = render_template("notworkingproducts.html",products = ProductModel.getNotWorkingProducts())
